...
## MỤC LỤC
1. [Cấu Hình RAM & Kết Nối](#1-cấu-hình-ram--kết-nối)
2. [Kích Hoạt Phần Cứng](#2-kích-hoạt-phần-cứng)
3. [Nhật Ký (Logging) & Tự Động Dọn Dẹp Rác](#3-nhật-ký-logging--tự-động-dọn-dẹp-rác)
4. [Chiến Lược Sao Lưu Tự Động](#4-chiến-lược-sao-lưu-tự-động)
5. [Hướng Dẫn Giả Lập Chịu Tải Cao](#5-hướng-dẫn-giả-lập-chịu-tải-cao)

---

## 1. Cấu Hình RAM & Kết Nối
...
## 4. Sao Lưu Tự Động (Backup Autopilot)
...
2. Tạo file kịch bản `test-tai.yml` bằng Notepad với nội dung gửi request liên tục trong 1 phút xuống API Backend:
   ```yaml
   config:
     target: "http://localhost:8080" # Đường dẫn API Backend
     phases:
       - duration: 60
